...
class FlyScanPositions():
    ...
    '''    
    def __init__(self, driver=Aerotech_Y, speed=1, start=0, end=1, delta=None, num_points=None):
        self.driver = driver
    ...
    def fcompute_senses(self):
        ...
        motor_dir = -1 if self.driver.motor.direction else 1
        ...
        self.overall_sense = self.user_direction * motor_dir * encoderDir
    ...
    def fcompute_positions_MCS(self):
        ...
        self.num_points = round(math.fabs(self.req_start-self.req_end) / self.delta_egu) + 2
        self.actual_end = self.req_start + (self.num_points - 2) * self.delta_egu
        end_spacing = self.delta_egu / 2.0 * self.user_direction
        self.PSO_positions = np.linspace(self.req_start - end_spacing, self.actual_end + end_spacing, self.num_points)
        print(self)
    
    def fcompute_positions_tomo(self):
        '''Computes several parameters describing the fly scan motion.
        These calculations are for tomography scans, where for N bins we need N pulses.
        Moreover, we base these on the number of points, not the delta between.
        ...
        #Compute the actual delta to keep things at an integral number of encoder counts
        self.delta_encoder_counts = round(abs(self.req_end - self.req_start) 
                                        / (self.num_points - 1) * self.driver.encoder_multiply)
        self.delta_egu = self.delta_encoder_counts / self.driver.encoder_multiply
        ...
        self.actual_end = self.req_start + (self.num_points - 1) * self.delta_egu * self.user_direction
        self.PSO_positions = np.linspace(self.req_start, self.actual_end, self.num_points)
        print(self)     
    ...
    def __str__(self):
        return '''
                    ...
                    EGU / Pulse = {3:f}'''.format(self.req_start, self.actual_end, 
                                    self.delta_encoder_counts, self.delta_egu, self.num_points)
    ...
    try:
        ...
        while True:
            ...
            if trigger_busy_PV.value == 1:
                ...
                fly_scan_pos = FlyScanPositions(driver=Aerotech_Theta, speed=speed_PV.value, 
                                    start=start_PV.value, end=end_PV.value, 
                ...
                if not Aerotech_Theta.motor.within_limits(fly_scan_pos.motor_start):
                    ...
                    raise ValueError
                if not Aerotech_Theta.motor.within_limits(fly_scan_pos.motor_end):
    ...
    while True:
        ...
        if repeated_scan_busy_PV.value == 1:
            ...
            for i in range(num_repeats):
                ...
                if func_args:
                    successful_scan = fly_scan_func(**func_args)
                else:
